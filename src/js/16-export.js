...
function _buildSheetRows(){
  ...
  var rows=[
    ...
    ['=== الملخص المالي ===',''],
    ['البيان','المبلغ (JD)'],
    ['إجمالي الإيرادات',t.rev],['نقد',t.cash],['فيزا',t.visa],
    ['إجمالي المصروفات',t.totalExp],['صافي الربح',t.profit],
    ['هامش الربح %',t.margin.toFixed(2)+'%'],
    ['الالتزامات المسددة',t.oblPaid],['رواتب مدفوعة',t.mSalPaid],['أجور يومية',t.dwTotal],
    [],
    ['=== ملخص المديرين ===','','','','',''],
    ['المدير','إيراد نقدي استلمه','مشتريات دفعها','التزامات دفعها','رواتب دفعها','الصافي'],
    ['يزن',t.yzR,t.yzP,t.yzO,t.yzS,t.yznNet],
    ['عبدالرحمن',t.abdR,t.abdP,t.abdO,t.abdS,t.abdNet],
    ['فيزا (رصيد)',t.visa,'','',t.visaSal,t.visaBalance],
    ['الربح النهائي الموحّد','','','','',t.finalProfit],
    [],
    ['=== المبيعات اليومية ===','','','','','',''],
    ['اليوم','نقد','فيزا','دفعات أخرى','الإجمالي','ملاحظات','استلمه'],
  ];
  d.sales.forEach(function(s){
    rows.push([s.day,s.cash||0,s.visa||0,s.pmts||0,(s.cash||0)+(s.visa||0)+(s.pmts||0),s.notes||'',s.receivedBy||'']);
  });
  rows.push([]);
  rows.push(['=== المشتريات ===','','','','','']);
  rows.push(['التاريخ','المورد','التصنيف','الوصف','المبلغ (JD)','قام بالشراء']);
  (d.purchases||[]).forEach(function(p){rows.push([p.date||'',p.sName||'',p.cat||'',p.desc||'',p.amt||0,p.buyer||'']);});
  rows.push([]);
  ...
  if(S.monthlyEmps&&S.monthlyEmps.length){
    ...
    rows.push(['الموظف','الأساسي','البدلات','الخصومات','الصافي','مسدد؟','مصدر الدفع']);
    S.monthlyEmps.forEach(function(e){var s=(d.mSal||{})[e.id]||{};rows.push([e.name,s.base||0,s.allow||0,s.ded||0,(s.base||0)+(s.allow||0)-(s.ded||0),s.paid?'نعم':'لا',s.paidBy||'']);});
  }
  return rows;
}
...
function exportExcel(){
  ...
  const rv=[["ريحانة كافيه — الإيرادات"],[],["اليوم","نقدي","فيزا","مدفوعات","صافي البيع","إجمالي الإيراد","ملاحظات","استلمه"]];
  let tC=0,tV=0,tP=0;
  d.sales.forEach(s=>{if((s.cash||0)+(s.visa||0)+(s.pmts||0)>0){const c=s.cash||0,v=s.visa||0,p=s.pmts||0;rv.push([`${s.day}/${m}`,c,v,p,c+v,c+v+p,s.notes||"",s.receivedBy||""]);tC+=c;tV+=v;tP+=p;}});
  rv.push(["المجموع",tC,tV,tP,tC+tV,tC+tV+tP,"",""]);
  const wsR=XLSX.utils.aoa_to_sheet(rv);wsR["!cols"]=[{wch:10},{wch:12},{wch:12},{wch:14},{wch:18},{wch:18},{wch:26},{wch:14}];XLSX.utils.book_append_sheet(wb,wsR,"الإيرادات");
  const pr=[["ريحانة كافيه — المشتريات"],[],["التاريخ","المورد","الفئة","البيان","المبلغ","الفاتورة","قام بالشراء"]];
  d.purchases.forEach(p=>pr.push([p.date,p.sName,catLbl(p.cat||"COGS"),p.desc||"",p.amt,p.inv||"",p.buyer||""]));
  pr.push(["الإجمالي","","","",d.purchases.reduce((s,p)=>s+p.amt,0),"",""]);
  const wsPr=XLSX.utils.aoa_to_sheet(pr);wsPr["!cols"]=[{wch:14},{wch:22},{wch:14},{wch:26},{wch:12},{wch:14},{wch:14}];XLSX.utils.book_append_sheet(wb,wsPr,"المشتريات");
  ...
  // ورقة ملخص المديرين
  const mg=[
    ["ريحانة كافيه — ملخص المديرين"],[`${mn} ${y}`],[],
    ["المدير","إيراد نقدي استلمه","مشتريات دفعها","التزامات دفعها","رواتب دفعها","الصافي"],
    ["يزن",t.yzR,t.yzP,t.yzO,t.yzS,t.yznNet],
    ["عبدالرحمن",t.abdR,t.abdP,t.abdO,t.abdS,t.abdNet],
    [],
    ["فيزا","مبيعات فيزا","رواتب من فيزا","","","رصيد الفيزا"],
    ["",t.visa,t.visaSal,"","",t.visaBalance],
    [],
    ["الربح النهائي الموحّد","","","","",t.finalProfit],
  ];
  const wsMg=XLSX.utils.aoa_to_sheet(mg);wsMg["!cols"]=[{wch:20},{wch:18},{wch:18},{wch:18},{wch:18},{wch:18}];XLSX.utils.book_append_sheet(wb,wsMg,"ملخص المديرين");
...
function exportPDF(){
  ...
  var salesRows=d.sales.filter(function(s){return(s.cash||0)+(s.visa||0)>0;}).map(function(s){
    ...
    var whoCls=who==='يزن'?'color:#0070F2;font-weight:700':who==='عبدالرحمن'?'color:#D97706;font-weight:700':'color:#aaa';
    return '<tr><td>'+s.day+'/'+m+'</td><td>'+n3(s.cash||0)+'</td><td>'+n3(s.visa||0)+'</td><td>'+n3((s.cash||0)+(s.visa||0))+'</td><td>'+(s.notes||'—')+'</td><td style="'+whoCls+'">'+who+'</td></tr>';
  }).join('');
  var purchRows=(d.purchases||[]).map(function(p){
    var who=p.buyer||'—';
    var whoCls=who==='يزن'?'color:#0070F2;font-weight:700':who==='عبدالرحمن'?'color:#D97706;font-weight:700':'color:#aaa';
    return '<tr><td>'+(p.date||'')+'</td><td>'+(p.sName||'')+'</td><td>'+n3(p.amt||0)+'</td><td>'+(p.desc||'—')+'</td><td style="'+whoCls+'">'+who+'</td></tr>';
  }).join('');
  // ملخص المديرين
  var t2=totals();
  var mgrRows=''
    +'<tr style="background:#f0f7ff"><td style="font-weight:700;color:#0070F2">👤 يزن</td>'
    +'<td>'+n3(t2.yzR)+'</td><td>'+n3(t2.yzP)+'</td><td>'+n3(t2.yzO)+'</td><td>'+n3(t2.yzS)+'</td>'
    +'<td style="font-weight:800;color:'+(t2.yznNet>=0?'#059669':'#DC2626')+'">'+n3(t2.yznNet)+'</td></tr>'
    +'<tr style="background:#fffbf0"><td style="font-weight:700;color:#D97706">👤 عبدالرحمن</td>'
    +'<td>'+n3(t2.abdR)+'</td><td>'+n3(t2.abdP)+'</td><td>'+n3(t2.abdO)+'</td><td>'+n3(t2.abdS)+'</td>'
    +'<td style="font-weight:800;color:'+(t2.abdNet>=0?'#059669':'#DC2626')+'">'+n3(t2.abdNet)+'</td></tr>'
    +'<tr style="background:#f5f5f5"><td style="font-weight:700;color:#7C3AED">💳 فيزا</td>'
    +'<td>'+n3(t2.visa)+'</td><td>—</td><td>—</td><td>'+n3(t2.visaSal)+'</td>'
    +'<td style="font-weight:800;color:'+(t2.visaBalance>=0?'#059669':'#DC2626')+'">'+n3(t2.visaBalance)+'</td></tr>';
  var html='<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>ريحانة كافيه — '+mn+' '+y+'</title>'
    ...
    +'</tbody></table>'
    +'<h2>ملخص المديرين</h2>'
    +'<table><thead><tr><th>المدير</th><th>إيراد نقدي استلمه</th><th>مشتريات دفعها</th><th>التزامات دفعها</th><th>رواتب دفعها</th><th>الصافي</th></tr></thead><tbody>'+mgrRows+'</tbody></table>'
    +'<table style="margin-top:6px"><thead><tr><th colspan="2">الربح النهائي الموحّد</th></tr></thead><tbody>'
    +'<tr class="profit"><td>يزن + عبدالرحمن + فيزا</td><td style="font-family:monospace;text-align:left">'+n3(t2.finalProfit)+' JD</td></tr>'
    +'</tbody></table>'
    +'<h2>المبيعات اليومية</h2><table><thead><tr><th>اليوم</th><th>نقد</th><th>فيزا</th><th>الإجمالي</th><th>ملاحظات</th><th>استلمه</th></tr></thead><tbody>'+salesRows+'</tbody></table>'
    +'<h2>المشتريات</h2><table><thead><tr><th>التاريخ</th><th>المورد</th><th>المبلغ</th><th>البيان</th><th>قام بالشراء</th></tr></thead><tbody>'+purchRows+'</tbody></table>'
    +'</body></html>';

  var jobName='ريحانة كافيه — تقرير '+mn+' '+y;
  // Android: PrintManager يولّد PDF حقيقي عبر نافذة الطباعة المدمجة
  if(window.AndroidStorage&&window.AndroidStorage.printHTML){
    window.AndroidStorage.printHTML(html, jobName);
    toast('⏳ جاري فتح نافذة الطباعة — اختر "حفظ كـ PDF"');
    log('تصدير PDF — '+mn+' '+y);
    return;
  }
